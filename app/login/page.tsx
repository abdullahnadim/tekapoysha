...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/dashboard"); // Successful login, send to dashboard
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
      <div className="max-w-md w-full bg-white p-8 sm:p-10 rounded-3xl shadow-sm border border-gray-100 animate-in fade-in zoom-in duration-500">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-500 font-medium text-sm mt-1">Log in to your TekaPoysha account.</p>
        </div>
        ...
        <div className="flex items-center my-6">
          ...
          <span className="px-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Or log in with email</span>
          <div className="flex-grow border-t border-gray-100"></div>
        </div>
  ...
  Don't have an account? <Link href="/register" className="text-blue-600 hover:text-blue-700 font-bold">Sign up</Link>
</p>
      </div>
    </div>
  );
