    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (data.type === "income") {
          ...
          const fromAcc = data.fromAccount as keyof typeof newBalances;
          const toAcc = data.toAccount as keyof typeof newBalances;
          if (fromAcc) newBalances[fromAcc] = (newBalances[fromAcc] || 0) - amount;
          if (toAcc) newBalances[toAcc] = (newBalances[toAcc] || 0) + amount;
        }
      });
            ...
            {transactions.slice(0, 5).map((txn) => (
              <div key={txn.id} className="flex justify-between items-center p-4 hover:bg-gray-50 rounded-lg transition-colors border border-gray-50">
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${txn.type === 'income' ? 'bg-green-100 text-green-600' : txn.type === 'transfer' ? 'bg-blue-100 text-blue-600' : 'bg-red-100 text-red-600'}`}>
                    {txn.type === 'income' ? 'np↓' : txn.type === 'transfer' ? '⇄' : '↑'}
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className={`font-bold ${txn.type === 'income' ? 'text-green-600' : txn.type === 'transfer' ? 'text-blue-600' : 'text-gray-900'}`}>
                      {txn.type === 'income' ? '+' : txn.type === 'transfer' ? '⇄' : '-'} ৳ {Number(txn.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                    </p>
                    {txn.description && (
                  ...
                  <button onClick={() => setEditingTxn({...txn, date: formatDateForInput(txn.date)})} className="text-gray-400 hover:text-blue-500 p-2">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                      <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                    </svg>
                  </button>

                  <button onClick={() => txn.id && handleDelete(txn.id)} className="text-gray-400 hover:text-red-500 p-2">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            ...
            <form onSubmit={handleUpdate} className="space-y-4">
              <div>
                ...
                <input type="number" required value={editingTxn.amount} onChange={(e) => setEditingTxn({ ...editingTxn, amount: Number(e.target.value) })} className="w-full rounded-lg border border-gray-300 px-3 py-2" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Account</label>
                  <select value={editingTxn.account} onChange={(e) => setEditingTxn({ ...editingTxn, account: e.target.value as any })} className="w-full rounded-lg border border-gray-300 px-3 py-2">
                ...
                <div>
                  ...
                  <input type="date" required value={editingTxn.date as string} onChange={(e) => setEditingTxn({ ...editingTxn, date: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
                <input type="text" value={editingTxn.description} onChange={(e) => setEditingTxn({ ...editingTxn, description: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2" />
              </div>
              <div className="flex gap-3 mt-6">
                <button type="button" onClick={() => setEditingTxn(null)} className="flex-1 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200">Cancel</button>
                <button type="submit" className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700">Save Changes</button>
              </div>
            </form>
          </div>
        </div>
